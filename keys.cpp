...
const MenuItem MENU_ITEMS[MENU_ITEM_COUNT] = {
  // idx 0-5: Keyboard (Key slots in both modes; min/max simple-only; job/wswitch sim-only)
  ...
  // idx 6-13: Mouse (duration/style/scroll simple-only; auto-clicks/click-type sim-only)
  ...
  // idx 14-16: Profiles (simple only — heading auto-hides in sim)
  ...
  // idx 17-22: Display (both modes; header display sim-only)
  ...
  // idx 25-33: Device (both modes; host OS sim-only)
