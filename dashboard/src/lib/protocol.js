...
export function parseSettings(data) {
  return {
    keyMin: parseInt(data.keyMin, 10) || 2000,
    keyMax: parseInt(data.keyMax, 10) || 6500,
    mouseJig: parseInt(data.mouseJig, 10) || 15000,
    mouseIdle: parseInt(data.mouseIdle, 10) || 30000,
    mouseAmp: parseInt(data.mouseAmp, 10) || 1,
    mouseStyle: parseInt(data.mouseStyle, 10) || 0,
    lazyPct: parseInt(data.lazyPct, 10) || 0,
    busyPct: parseInt(data.busyPct, 10) || 0,
    dispBright: parseInt(data.dispBright, 10) || 80,
    saverBright: parseInt(data.saverBright, 10) || 20,
    saverTimeout: parseInt(data.saverTimeout ?? '5', 10),
    animStyle: parseInt(data.animStyle ?? '2', 10),
    dispFlip: parseInt(data.dispFlip, 10) || 0,
    name: data.name || 'GhostOperator',
    btWhileUsb: parseInt(data.btWhileUsb, 10) || 0,
    scroll: parseInt(data.scroll, 10) || 0,
    dashboard: parseInt(data.dashboard, 10) || 0,
    invertDial: parseInt(data.invertDial, 10) || 0,
    decoy: parseInt(data.decoy, 10) || 0,
    schedMode: parseInt(data.schedMode, 10) || 0,
    schedStart: parseInt(data.schedStart ?? '108', 10),
    schedEnd: parseInt(data.schedEnd ?? '204', 10),
    slots: (data.slots || '2,28,28,28,28,28,28,28').split(',').map(Number),
    // Simulation mode settings
    opMode: parseInt(data.opMode, 10) || 0,
    jobSim: parseInt(data.jobSim, 10) || 0,
    jobPerf: parseInt(data.jobPerf ?? '5', 10),
    jobStart: parseInt(data.jobStart ?? '96', 10),
    phantom: parseInt(data.phantom, 10) || 0,
    clickSlots: (data.clickSlots || '1,7,7,7,7,7,7').split(',').map(Number),
    winSwitch: parseInt(data.winSwitch, 10) || 0,
    switchKeys: parseInt(data.switchKeys, 10) || 0,
    headerDisp: parseInt(data.headerDisp, 10) || 0,
    // Sound settings
    sound: parseInt(data.sound, 10) || 0,
    soundType: parseInt(data.soundType, 10) || 0,
    sysSounds: parseInt(data.sysSounds, 10) || 0,
    // Volume control settings
    volumeTheme: parseInt(data.volumeTheme, 10) || 0,
    encButton: parseInt(data.encButton, 10) || 0,
    sideButton: parseInt(data.sideButton, 10) || 0,
    // Game settings
    ballSpeed: parseInt(data.ballSpeed, 10) || 0,
    paddleSize: parseInt(data.paddleSize, 10) || 0,
    startLives: parseInt(data.startLives, 10) || 0,
    highScore: parseInt(data.highScore, 10) || 0,
    snakeSpeed: parseInt(data.snakeSpeed, 10) || 0,
    snakeWalls: parseInt(data.snakeWalls, 10) || 0,
    snakeHiScore: parseInt(data.snakeHiScore, 10) || 0,
    racerSpeed: parseInt(data.racerSpeed, 10) || 0,
    racerHiScore: parseInt(data.racerHiScore, 10) || 0,
    // Shift/lunch settings
    shiftDur: parseInt(data.shiftDur ?? '480', 10),
    lunchDur: parseInt(data.lunchDur ?? '30', 10),
    // Lifetime stats (read-only)
    totalKeys: parseInt(data.totalKeys, 10) || 0,
    totalMousePx: parseInt(data.totalMousePx, 10) || 0,
    totalClicks: parseInt(data.totalClicks, 10) || 0,
  }
}
...
export function parseStatus(data) {
  return {
    ...
    kb: data.kb === '1',
    ms: data.ms === '1',
    bat: parseInt(data.bat, 10) || 0,
    batMv: parseInt(data.batMv, 10) || 0,
    profile: parseInt(data.profile ?? '1', 10),
    mode: parseInt(data.mode, 10) || 0,
    mouseState: parseInt(data.mouseState, 10) || 0,
    uptime: parseInt(data.uptime, 10) || 0,
    kbNext: data.kbNext || '---',
    timeSynced: data.timeSynced === '1',
    daySecs: parseInt(data.daySecs, 10) || 0,
    schedSleeping: data.schedSleeping === '1',
    // Simulation mode runtime status (only present when opMode=1)
    simBlock: parseInt(data.simBlock, 10) || 0,
    simMode: parseInt(data.simMode, 10) || 0,
    simPhase: parseInt(data.simPhase, 10) || 0,
    simProfile: parseInt(data.simProfile, 10) || 0,
    // Lifetime stats (live)
    totalKeys: parseInt(data.totalKeys, 10) || 0,
    totalMousePx: parseInt(data.totalMousePx, 10) || 0,
    totalClicks: parseInt(data.totalClicks, 10) || 0,
...
export function parseWorkMode(data) {
  ...
  return {
    idx: parseInt(data.idx, 10) || 0,
    name: data.name || '???',
    kb: parseInt(data.kb, 10) || 0,
    wL: parseInt(data.wL, 10) || 0,
    wN: parseInt(data.wN, 10) || 0,
    wB: parseInt(data.wB, 10) || 0,
    timing: [parseTiming(data.t0), parseTiming(data.t1), parseTiming(data.t2)],
    dMin: parseInt(data.dMin, 10) || 0,
    dMax: parseInt(data.dMax, 10) || 0,
    pMin: parseInt(data.pMin, 10) || 0,
    pMax: parseInt(data.pMax, 10) || 0,
  }
}
...
export function parseSimBlocks(data) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const durMin = parseInt(parts[2], 10) || 0
    const modes = []
