...
- `!serialdfu` BLE UART command writes `0x4E` to `GPREGRET`, resets into USB CDC serial DFU mode
- Web dashboard "Firmware Update" section orchestrates the full flow: BLE command → reboot → Web Serial API transfer
...
| 1.7.2 | Web Serial DFU — browser-based firmware updates via USB serial |
...
| `dashboard/` | Vue 3 web dashboard (Vite + Web Bluetooth + Web Serial DFU) |
