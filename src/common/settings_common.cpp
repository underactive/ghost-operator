...
// ============================================================================
// Portable settings logic — getters, setters, defaults, formatting
// Platform-specific load/save/temperature in platform settings_*.cpp
// ============================================================================
...
void loadDefaults() {
  ...
  settings.clickSlots[0] = 1;     // Middle
  for (int i = 1; i < NUM_CLICK_SLOTS; i++) {
    settings.clickSlots[i] = NUM_CLICK_TYPES - 1;  // NONE
  }
...
void formatMenuValue(uint8_t settingId, MenuValueFormat format, char* buf, size_t bufSize) {
  ...
  switch (format) {
    ...
    case FMT_CLICK_TYPE:    snprintf(buf, bufSize, "%s", (val < NUM_CLICK_TYPES) ? CLICK_TYPE_NAMES[val] : "???"); return;
